...
function rowClick(row) {
    ...
    //alert("ID : " + rowId);
    window.location = "vf_edit_workload.aspx?cc=" + rowId + "&sc=" + semester + "&yc=" + year_code;
}

function edit_course_rowClick(row) {
    var rowId = row.parentElement.parentElement.parentElement.childNodes[0].childNodes[0].nodeValue;
    //alert("ID : " + rowId);
   // window.location = "vf_edit_workload.aspx?cc=" + rowId + "&sc=" + semester + "&yc=" + year_code;
    location.replace("frmcoursemaster.aspx?c=" + rowId + "&s=" + semester + "&y=" + year_code + "");
}
var semester = '';
var year_code = '';
var prog_code = '';
...
function get_vf_workload_detail() {
    ...
    $.ajax(
    {
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../../WebService.asmx/get_vf_workload_detail",
        //async: false,
...
//        columns.push({ "sTitle": attr, "mData": attr });
    ...
    var edit_column = { "sTitle": "Edit", "mData": null, "bSortable": false, "mRender": function (course_code) {
        return '<center><button type="button" onclick="rowClick(this)">Edit</button></center>';
    } 
    };

    columns.push(edit_column);
    var course_edit_column = {
        "sTitle": "Edit Course", "mData": null, "bSortable": false, "mRender": function (course_code) {
            return '<center><button type="button" onclick="edit_course_rowClick(this)">Edit Course</button></center>';
        }
    };

    columns.push(course_edit_column);
