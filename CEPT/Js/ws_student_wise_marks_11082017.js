...
$(document).ready(function () {
    ...
    if ($("#hdnusertype").val() == 'WSA') {
        //$('#td_btn_submit').css('display', 'none');
    }
    ...
    if (course_exams.length > 0) {
        for (var i = 0; i < course_exams.length; i++) {
            str = str + '<div><div id="div_' + course_exams[i]['exam_code'] + '" style="width: 110px;font-size:14px;display:inline-block;margin-left: 5px;">Assessment ' + (i + 1) + ' </div>' +
                    ...
                    '<input id="txt_weightage_' + course_exams[i]['exam_code'] + '" type="text" value="' + course_exams[i]['weightage'] + '" placeholder="Weightage" class="" onkeypress="return IsNumeric(event);" style="width:50px;margin-left: 5px;"/></div>';
        }
    }
    else {
        str = str + '<div><div id="div_exam_1" style="width: 110px;font-size:14px;display:inline-block;margin-left: 5px;">Assessment 1 </div>' +
                    ...
                    '<input id="txt_weightage_exam_1" type="text" value="" placeholder="Weightage" class="" onkeypress="return IsNumeric(event);" style="width:50px;margin-left: 5px;"/></div>';
    }
    $('.modal-body').html(str);
...
function add_exam_row() {
    ...
    if ($('.modal-body')[0].childNodes.length < 10) {
        ...
        //            var modal_row = { 'title': '', 'weightage': '' }
        //            modal_row['title'] = $('#txt_title_exam_' + (i + 1)).val();
        //            modal_row['weightage'] = $('#txt_weightage_exam_' + (i + 1)).val();

        //            modal_data.push(modal_row);
        //        }
        ...
        str = str + '<div><div id="div_exam_' + ($('.modal-body')[0].childNodes.length + 1) + '" style="width: 110px;font-size:14px;display:inline-block;margin-left: 5px;">Assessment ' + ($('.modal-body')[0].childNodes.length + 1) + ' </div>' +
                    ...
                    '<input id="txt_weightage_exam_' + ($('.modal-body')[0].childNodes.length + 1) + '" type="text" value="" placeholder="Weightage" class="" onkeypress="return IsNumeric(event);" style="width:50px;margin-left: 5px;"/></div>';

        //$('.modal-body').html($('.modal-body').html() + str);
        $('.modal-body').html(str);

        //        for (var i = 0; i < modal_data.length; i++) {
        //            $('#txt_title_exam_' + (i + 1)).val(modal_data[i]['title'].value);
        //            $('#txt_weightage_exam_' + (i + 1)).val(modal_data[i]['weightage'].value);
        //        }
    }
    else {
            ...
            $.ajax(
                {
                    ...
                    url: "../../WebService.asmx/ws_remove_all_exam_dtl",
                    //async: false,
...
function updateColumn() {
    ...
    if (modal_data.length <= 10) {
        $.ajax(
        {
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: "../../WebService.asmx/ws_course_wise_all_exam_add",
            //async: false,
    ...
    if ($('#txt_modal_examtitle').val() != '') {
        if (table_headers.length < 12) {
            ...
            $.ajax(
            {
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "../../WebService.asmx/ws_course_wise_exam_add",
                //async: false,
            ...
            if (data.d != "" && data.d != "[]") {
                if (data.d == 'Data Saved Successfully') {
                    bootbox.alert('Course submitted successfully', function (result) {
                        window.location = "WS_Course_wise_entered_marks.aspx";
                    });
                }
                else {
                    submit_flag = false;
...
function save_student_marks_Data() {
    ...
    $.ajax(
    {
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../../WebService.asmx/ws_save_all_students_marks",
        //async: false,
...
function rowSave(row) {
    ...
    $.ajax(
    {
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../../WebService.asmx/ws_save_student_marks",
        //async: false,
    ...
    $.ajax(
    {
        ...
        success: function (data) {
            ...
            if (data.d != "" && data.d != "[]") {
                ...
                table_headers.push({ "sTitle": "Attendance", "mData": "attendance", "bSortable": false, mRender: function (ddata) {
                    return "<input type='text' value='" + ddata + "' class='inline_input' onkeypress='return IsNumeric(event);' disabled/>";
                }
                });
                ...
                for (var i = 0; i < course_exams.length; i++) {
                    ...
                    var header_def = { "sTitle": course_exams[i].exam_title + ' (' + course_exams[i].weightage + '%)', "mData": course_exams[i].exam_code, "bSortable": false, mRender: function (ddata) {
                        //return "<input type='text' value='" + ddata + "' class='inline_input' style='display:none;' onkeypress='return IsNumeric(event);'/>" +
...
function course_wise_student_marks() {
    ...
    if (calc_flag) {
        ...
        ajax_url = "../../WebService.asmx/ws_get_student_wise_grade";
        calc_flag = false;
    }
    else {
        ajax_url = "../../WebService.asmx/ws_course_wise_student_marks_list";
    }
...
function student_wise_marks_list(data) {
    ...
    oTable = $("#example").dataTable({
        ...
        //    //"sButtonText": 'Export',
        //    //"aButtons": ["xls"]
        //    ////"aButtons": [{ "sExtends": "csv", "fnCellRender": function (sValue, iColumn, nTr, iDataIndex) {
        //    ////if(iColumn == 2) {
        //    ////    return nTr.children[iColumn].children[0].value;
        //    ////}
        //    ////else if (iColumn > 2) {
        //    ////    if (nTr.children[iColumn].children[1].value != '') return nTr.children[iColumn].children[1].value;
        //    ////    else return nTr.children[iColumn].children[0].value;
        //    ////}
        //    ////else return sValue;
        //    ////}
        //    ////}, "xls"]
    ...
    if (student_marks.length > 0) {
        ...
        $("#example tbody tr").each(function (i) {
            if (course_exams.length > 0) {
                ...
                if ($(this).children().eq(3)[0].children[0].value == "75") $(this).children().eq(3)[0].children[1].value = "P";
                if ($(this).children().eq(3)[0].children[0].value == "25") $(this).children().eq(3)[0].children[1].value = "NP";
                if (course_exams.length > 1) {
                    ...
                    if ($(this).children().eq(4)[0].children[0].value == "75") $(this).children().eq(4)[0].children[1].value = "P";
                    if ($(this).children().eq(4)[0].children[0].value == "25") $(this).children().eq(4)[0].children[1].value = "NP";
                    if (course_exams.length > 2) {
                        ...
                        if ($(this).children().eq(5)[0].children[0].value == "75") $(this).children().eq(5)[0].children[1].value = "P";
                        if ($(this).children().eq(5)[0].children[0].value == "25") $(this).children().eq(5)[0].children[1].value = "NP";
                        if (course_exams.length > 3) {
                            ...
                            if ($(this).children().eq(6)[0].children[0].value == "75") $(this).children().eq(6)[0].children[1].value = "P";
                            if ($(this).children().eq(6)[0].children[0].value == "25") $(this).children().eq(6)[0].children[1].value = "NP";
                            if (course_exams.length > 4) {
                                ...
                                if ($(this).children().eq(7)[0].children[0].value == "75") $(this).children().eq(7)[0].children[1].value = "P";
                                if ($(this).children().eq(7)[0].children[0].value == "25") $(this).children().eq(7)[0].children[1].value = "NP";
                                if (course_exams.length > 5) {
                                    ...
                                    if ($(this).children().eq(8)[0].children[0].value == "75") $(this).children().eq(8)[0].children[1].value = "P";
                                    if ($(this).children().eq(8)[0].children[0].value == "25") $(this).children().eq(8)[0].children[1].value = "NP";
                                    if (course_exams.length > 6) {
                                        ...
                                        if ($(this).children().eq(9)[0].children[0].value == "75") $(this).children().eq(9)[0].children[1].value = "P";
                                        if ($(this).children().eq(9)[0].children[0].value == "25") $(this).children().eq(9)[0].children[1].value = "NP";
                                        if (course_exams.length > 7) {
                                            ...
                                            if ($(this).children().eq(10)[0].children[0].value == "75") $(this).children().eq(10)[0].children[1].value = "P";
                                            if ($(this).children().eq(10)[0].children[0].value == "25") $(this).children().eq(10)[0].children[1].value = "NP";
                                            if (course_exams.length > 8) {
                                                ...
                                                if ($(this).children().eq(11)[0].children[0].value == "75") $(this).children().eq(11)[0].children[1].value = "P";
                                                if ($(this).children().eq(11)[0].children[0].value == "25") $(this).children().eq(11)[0].children[1].value = "NP";
                                                if (course_exams.length > 9) {
                                                    ...
                                                    if ($(this).children().eq(12)[0].children[0].value == "75") $(this).children().eq(12)[0].children[1].value = "P";
                                                    if ($(this).children().eq(12)[0].children[0].value == "25") $(this).children().eq(12)[0].children[1].value = "NP";
                                                }
                                            }
                                        }
                                    }
...
function display_excel_format() {
   ...
   // table_headers_xls = [{ "sTitle": "Student_Code", "mData": "student_code", "bSortable": false}];
    table_headers_xls = [{
        "sTitle": "Student_Code", "mData": null, "bSortable": false, mRender: function (data) {
    ...
    if (course_exams.length > 0) {
        //table_headers_xls.push({ "sTitle": "Attendance", "mData": null, "bSortable": false });
        ...
        for (var i = 0; i < course_exams.length; i++) {
            //table_headers_xls.push({ "sTitle": course_exams[i].exam_title, "mData": null, "bSortable": false });
            table_headers_xls.push({
