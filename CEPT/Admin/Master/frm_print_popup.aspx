<%@ Page Language="C#" AutoEventWireup="true" CodeFile="frm_print_popup.aspx.cs"
    Inherits="Admin_Master_frm_print_popup" %>
...
<![endif]-->
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $.ajax({
                type: "POST",
                url: "../../WebService.asmx/get_user_data_for_pay_slip_for_manually_student_wise",
                data: "{'student':'" + student + "','fees_status':'" + fees + "'}",
                contentType: "application/json",
                datatype: "json",
                async: false,
                success: function (data) {
                    if (data.d != "") {
                        if (data.d == "Problem in Generate Payslip Number.") {
                            alert(data.d);
                            return false;
                        }

                        if (data.d == "student data not found.") {
                            alert(data.d);
                            return false;
                        }

                        if (data.d == "gender") {
                            alert("Gender is not found for selected student");
                            return false;
                        }

                        if (data.d == "fees_detail") {
                            alert("Fees detail not found");
                            return false;
                        }
                        ...
                        ////$('.lbl_student_code').text(user_data[0]["user_id"]);
                        ////$('.lbl_name_of_branch').text(user_data[0]["dept_name"]);
                        //$('.lbl_name_of_institute').text("CEPT FACULTY OF ARCHITECTURE");
                        //$('.lbl_pan_no').text('AAAJC0452C');
                        ////$('.lbl_sem_code').text(user_data[0]["semester_code"]);
                        ////$('.lbl_payslip_number').text(user_data[0]["payslip_number"]);
                        //$('.lbl_bank_code').text("FCCFAR");
                        //////

                        $('#main_fees').css('display', 'block');

                        if (amount.trim() == '') {
                            $('.lbl_amount1').text(parseFloat(user_data[0]["amount"]));
                            var rupees = convert_number(parseFloat(user_data[0]["amount"]));
                            $('.lbl_amount_in_word1').text(rupees + ' ' + 'Only');
                        }
                        else {
                            $('.lbl_amount1').text(parseFloat(amount));
                            var rupees = convert_number(parseFloat(amount));
                            $('.lbl_amount_in_word1').text(rupees + ' ' + 'Only');
                        }
                    }
                    else {
...
<body>
    ...
    <div id="div_fees" style="display: block">
        <div id="main_fees" align="center" x:publishsource="Excel" style="display: block;
