...
public partial class Admin_Master_student_wise_session_attendence : System.Web.UI.Page
{
    string course_code = "";
    string year_type = "";
    string semester = "";
            ...
            if (dts != null)
            {
                ...
                foreach (DataRow dr in dts.Rows)
                {
                    for (i = 0; i < 1; i++)
                    {
                        ...
                        dt.Rows.Add(workRow);
                        //workRow[0] = dr["sca_user_id"];//dr[1].ToString();
                        //workRow[1] = dr["session_name"];//dr[1].ToString();
                        //string[] arr_date = dr["session_date"].ToString().Split('/');
                        //workRow[2] = arr_date[1] + "/" + arr_date[0] + "/" + arr_date[2].Substring(0,4);//dr[1].ToString();
                        //if(dr["session_date"].ToString() != "")
                        //{
                        //    workRow[3] = dr["session_attended"].ToString();
                        //}
                        //dt.Rows.Add(workRow);
                    }
                }
