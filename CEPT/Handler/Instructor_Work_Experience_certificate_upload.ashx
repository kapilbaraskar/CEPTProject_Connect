<%@ WebHandler Language="C#" Class="Instructor_Work_Experience_certificate_upload" %>

using System;
using System.Web;

public class Instructor_Work_Experience_certificate_upload : IHttpHandler,System.Web.SessionState.IReadOnlySessionState {
    ...
    public void ProcessRequest(HttpContext context)
    {
        try
        {
            ...
            string Serverpath = System.Configuration.ConfigurationManager.AppSettings["InstructorWorkExperiencecertificateuploadPath"];
            
            if (context.Request.Files.Count > 0)
            {
                var postedFile = context.Request.Files[0];
                string filesize = System.Configuration.ConfigurationManager.AppSettings["InstructorWorkExperiencecertificateuploadMaxSize"];
                ...
                if (mFileSize <= Convert.ToInt32(filesize))
                {
                   ...
                   int random_no = random.Next(1000, 9999);
                    //Portfolio
                    string[] extension = postedFile.ContentType.Split(new char[] { '/' });
                    //finalFileName = user_id + "_" + file;
                    finalFileName = user_id + "_Work_Experience_certificate_" +random_no+ "." + extension[1];
