...
//		}
		...
		"sButtonText": "Excel",
		"fnClick": function( nButton, oConfig, flash ) {
            var sliced_id = nButton.id.slice(11);
			download_excel(sliced_id.substr(0,sliced_id.length-2));
		}
	} ),
...
function sheet_from_array_of_arrays(data, opts) {
    ...
    for (var R = 0; R != data.length; ++R) {
        for (var C = 0; C != 31; ++C) {
            ...
            var cell = {
                //v: data[R][C]
                v: (R == 0) ? data[R][C] : data[R][data[0][C]]
            };
...
function download_excel(table) {
    ...
    if (objTable == undefined) { try { if (oTable[0].id == table) { objTable = oTable; } } catch (err) { } }
    if (objTable == undefined) { try { if (oTable2[0].id == table) { objTable = oTable2; } } catch (err) { } }
    if (objTable == undefined) { try { if (oTable3[0].id == table) { objTable = oTable3; } } catch (err) { } }
    if (objTable == undefined) { try { if (oTable4[0].id == table) { objTable = oTable4; } } catch (err) { } }
    if (objTable == undefined) { try { if (oTable5[0].id == table) { objTable = oTable5; } } catch (err) { } }

    if (objTable != undefined) {
        //var data = oTable.fnGetData();
        var data = objTable.fnGetData();
