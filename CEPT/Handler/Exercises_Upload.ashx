<%@ WebHandler Language="C#" Class="Exercises_Upload" %>

using System;
using System.Web;

public class Exercises_Upload : IHttpHandler, System.Web.SessionState.IRequiresSessionState {
    int mFileSize = 0;
    string msg = "";
    ...
    public void ProcessRequest (HttpContext context) {
        try
        {
            ...
            string Serverpath = System.Configuration.ConfigurationManager.AppSettings["ExercisesUploadPath"];

            if (context.Request.Files.Count > 0)
            {
                var postedFile = context.Request.Files[0];
                string filesize = System.Configuration.ConfigurationManager.AppSettings["ExercisesUploadMaxSize"];
                mFileSize = postedFile.ContentLength;
                string course_code = "";

                if (context.Request.Form["CourseCode"] != null)
                {
                    course_code = context.Request.Form["CourseCode"].ToString();
                }
                ...
                if (mFileSize <= Convert.ToInt32(filesize))
                {
                    ...
                    finalFileName = course_code + "." + extension[1];
