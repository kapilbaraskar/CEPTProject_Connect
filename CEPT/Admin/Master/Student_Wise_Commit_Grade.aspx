<%@ Page Title="Student Wise Commit Grade" Language="C#" MasterPageFile="~/AdminCEPT.master" AutoEventWireup="true" CodeFile="Student_Wise_Commit_Grade.aspx.cs" Inherits="Admin_Master_Student_Wise_Commit_Grade" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>
                <i class="icon-desktop"></i>&nbsp;Verify Grade Student Wise<%--Student Wise Commit Grade--%>
            </h1>
        </div>
    </div>
            ...
            <%--<div class="panel-heading">
                <strong>Student List</strong> <span style="float: right;">
                    <asp:Button ID="btn_download_all" class="btn btn-primary" runat="server" Text="Download All"
                        OnClick="Button1_Click" Style="height: 40px; margin-top: -10px;" />
                </span>
            </div>--%>
            <div>
                <%--class="panel-body"--%>
    ...
    <%--<input type="hidden" id="hdn_download" runat="server" clientidmode="Static" onclick="Download_Student_Grade_Report"/>--%>
    ...
    <script type="text/javascript">
        ...
        function retrieve_Student_Data() {
            ...
            var filter_criteria = { sem_code: semester, year: year_code, dept: dept_code, prog: prog_code, course_code: course_code };
            ...
            $.ajax(
            {
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "../../WebService.asmx/get_gradereport_student_data_commit_grade",
                data: "{sem_code:'" + semester + "' , year_code : '" + year_code + "',dept_code: '" + dept_code + "',prog_code: '" + prog_code + "',prog_level_code:'" + prog_level_code + "',year_of_enrollment:'" + year_of_enrollment + "',course_code:'" + course_code + "'}",
                dataType: "json",
                success: function (data) {
        ...
        function display_Student_Data(data) {
            ...
            oTable = $("#example").dataTable({
                ...
                //{ "sTitle": "Course Credits", "mData": "course_credits", "bSortable": false }
                    ...
                    {"sTitle": "Grade", "mData": null, "bSortable": false, "mRender": function (course_code) {
                        //alert(course_code);
                        return '<center><button type="button" onclick="rowClick_verify(this)">Verify</button></center>';
                    }
                },
                ]
            });
        ...
        function rowClick(row) {
            ...
            //var str_url = "https://localhost:3787/CEPT/Admin/Report/GradeReportPDF2.aspx?uid=" + rowId + "&sem=" + $('#drpsemester').val() + "&year=" + $('#drpyear').val();

            //window.open(str_url,'_blank');

            var filter_criteria = { sem_code: semester, year: year_code, dept: dept_code, prog: prog_code, student_code: rowId };
            $('#hdn_filter').val(JSON.stringify(filter_criteria));

            $('#hdn_download').click();
        }

        function rowClick_verify(row) {
            debugger;

            var rowId = row.parentElement.parentElement.parentElement.childNodes[0].childNodes[0].nodeValue;

            var url = "Verify_Grade_Student_Wise.aspx?StudentID=" + rowId + "&Sem=" + semester + "&Year=" + year_code;
