        ...
        $.ajax(
            {
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "../../WebService.asmx/Get_saved_selected_course_data_CFP_for_report",
                data: "{sem_code:'" + semester + "' , year_code :'" + year_code + "'}",
                dataType: "json",
                success: function (data) {
                    if (data.d != "") {
...
function display_total_selected_course_data(data) {
    ...
    $('#btn_cfp_publish').css('display', 'block');
...
function save_data_CFP_logic() {
    ...
    var CFP_CODE = $('#drpcoursetype').val();
    if (CFP_CODE == "") {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../../WebService.asmx/Save_assign_course_dtl_for_CFP_logic",
        data: "{sem_code:'" + semester + "' , year_code : '" + year_code + "'}",
        dataType: "json",
        success: function (data) {
            if (data.d != "") {
...
function display_student_Course_allocation(data) {
    ...
    ModifiedData.forEach(function (CurrentItem) {
        CurrentItem.course_code = CurrentItem.course_code.replace("Span", "Sppan");
        CurrentItem.course_code = CurrentItem.course_code.replace("span", "Sppan");
    });
    oTable = $("#example").dataTable({
        "bPaginate": true,
    ...
    Array.from($("#example_paginate > ul > li")).forEach(function (PaginationButton) {
        PaginationButton.onclick = function () {
            Array.from($('td:only-child')).forEach(function (TdElement) {
                if (TdElement.textContent.includes("Sppan")) {
                    TdElement.textContent = TdElement.textContent.replace("Sppan", "Span");
                }
            })
        }
    });
...
//            //{ "sTitle": "Course Code", "mData": "course_code", "bSortable": false, "bVisible": false },
//            {
//                "sTitle": "Course Code", "mData": "course_code", "bSortable": false, mRender: function (data) {
//                    var sss = 'Span and Space: Exploring Tectonics in Architecture';
//                    if (data == sss)
//                    {
//                        return 'Sppan and Space: Exploring Tectonics in Architecture';
//                    }
//                    else { return data; }
...
function Display_Assigned_report(data) {
    ...
    oTable = $("#example").dataTable({
        ...
        "aoColumns": [
            ...
            { "sTitle": "Elective Course", "mData": "elective_course", "bSortable": false },
            { "sTitle": "Elective Credits", "mData": "elective_credits", "bSortable": false },
            { "sTitle": "Batch Year", "mData": "BachCode", "bSortable": false },
            { "sTitle": "Admission Year", "mData": "AdmissionYear", "bSortable": false }
        ]
    });
...
function fees_payment_report_data_new() {
    ...
    $.ajax(

        {
            ...
            success: function (data) {
                ...
                if (data.d != "") {
                    Display_fees_payment_report_data_new(data.d);
                }
                else {
                    bootbox.alert('There is No data Found For Selected Semester');
                    return false;
...
function Display_fees_payment_report_data_new(data) {
    $('#DataList').css('display', 'block');
    ...
    oTable = $("#example").dataTable({
        ...
        "aoColumns": [
            ...
            { "sTitle": "Fees Paid", "mData": "Fees_Paid", "bSortable": false },
            { "sTitle": "Scholarship Amount", "mData": "Scholarship_Approved", "bSortable": false },
            { "sTitle": "Total Outstanding Amount", "mData": "Outstanding_fees", "bSortable": false },
...
function bindyeardata_for_batch() {
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../../WebService.asmx/Get_year_data",
        ...
        success: function (data) {
            if (data.d != "") {
                ...
                $('#BthYear').empty().append($("<option></option>").val("").html("-- Please Select Year --"));
                for (var i = 0; i < year_data.length; i++) {
                    $('#BthYear').append($("<option></option>").val(year_data[i]["year_code"]).html(year_data[i]["year_desc"]));
                }
                $('#BthYear').chosen();
            }
        },
...
function bindTAType() {
    ...
    $('#drptype').append($("<option></option>").val("23").html("Studio"));
    $('#drptype').append($("<option></option>").val("24").html("Course"));
