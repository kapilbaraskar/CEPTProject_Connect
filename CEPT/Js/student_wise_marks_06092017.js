...
$(document).ready(function () {
    ...
    if (CC == "CFP001" || CC == "CFP002" || CC == "CFP003" || CC == "CFP004" || CC == "CFP005" || CC == "CFP006" || CC == "CFP007" || CC == "CFP008" || CC == "CFP009" || CC == "CFP010") {
        $("#btn_calc_grade").remove();
    }
...
function addModalData() {
    ...
    if (course_exams.length > 0) {
        ...
        str = str + '<div><div id="div_exam_1" style="width: 110px;font-size:14px;display:inline-block;margin-left: 5px;">Assessment 1 </div>' +
                    ...
                    '<select id="drp_exam_type_exam_1" style="width:110px;margin-left: 5px;">' + drp_exam_criteria + '</select></div>';
    }
    $('.modal-body').html(str);
...
function add_exam_row() {
    ...
    if ($('.modal-body')[0].childNodes.length < 10) {
        ...
        str = str + '<div><div id="div_exam_' + ($('.modal-body')[0].childNodes.length + 1) + '" style="width: 110px;font-size:14px;display:inline-block;margin-left: 5px;">Assessment ' + ($('.modal-body')[0].childNodes.length + 1) + ' </div>' +
                    ...
                    '<select id="drp_exam_type_exam_' + ($('.modal-body')[0].childNodes.length + 1) + '" style="width:110px;margin-left: 5px;">' + drp_exam_criteria + '</select></div>';

        //$('.modal-body').html(str);
        $('.modal-body').append(str);

        var str_msg = '<h4 style="width: 530px;text-align:center;"><b>Note</b></h4><br/>';
        str_msg += '<p> 1. Please create assessments and select appropriate type from the drop down.</p>';
        str_msg += '<p> 2. MID TERM assessments are of type "Mid Term"</p>';
        str_msg += '<p> 3. Other INTERNAL assessments, which are not part of Mid Term assessment, are of type "Internal"</p>';
        str_msg += '<p> 4. The FINAL assessment of the course, is of type "Final/External"</p>';
        str_msg += '<p> 5. For each type, there can be more than one assessments.</p>';
        str_msg += '<p> 6. Please note that the results publication process runs based on the Assessment Type, and not based on the name of the assessment.</p>';
        str_msg += '<p> 7. Mid term results of a course are published ONLY if marks are submitted for ALL the assessments of type "Mid Term".</p>';
        if (norepeat_msg == 0) {
            bootbox.alert('' + str_msg + '');
        }
        norepeat_msg += 1;
    }
    else {
...
function updateColumn() {
    ...
    if (modal_data.length <= 10) {
        for (var i = 0; i < modal_data.length; i++) {
            ...
            if ($('#drp_exam_type_exam_' + (i + 1)).val() == '') {
                bootbox.alert('Please Select Exam Type');
                return false;
            }
...
function calculate_grade() {
    ...
    if ($("#example_filter :input").val() != '') {
        bootbox.alert("Please Remove Student Code in Search Box.");
        return false;
    }
...
function submit_grade() {
    ...
    for (var r = 0; r < course_exams.length; r++)
    {
        if (course_exams[r]["is_submit"] != "Y")
        {
            if (submit) {
                submit = false;
            }
    ...
    if (submit) {
        ...
        bootbox.alert('Please submit Assessment.');
    }
}
...
function save_student_marks_Data() {
    ...
    $("#example tbody tr").each(function (i) {
        ...
        editor1.attendance = $(this).children().eq(2)[0].children[0].value.trim();
    ...
    $.ajax(
    {
        ...
        success: function (data) {
            if (data.d != "" && data.d != "[]") {
                ...
                drp_exam_criteria = '';
                drp_exam_criteria += '<option value="">Select Type</option>';
                for (var i = 0; i < obj_exam_criteria.length; i++) {
        ...
        success: function (data) {
            ...
            if (data.d != "" && data.d != "[]") {
                ...
                for (var i = 0; i < course_exams.length; i++) {
                    ...
                    var header_def = { "sTitle": course_exams[i].exam_title + ' (' + course_exams[i].weightage + '%)<br/><button id="btn_' + course_exams[i].exam_code + '" type="button" class="btn btn-small btn-primary" style="margin-top:5px;" onclick="submit_exam(\'' + course_exams[i].exam_code + '\')">Submit</button>', "mData": course_exams[i].exam_code, "bSortable": false, mRender: function (ddata) {
                        return "<input type='text' value='" + ddata + "' class='inline_input' onkeypress='return IsNumeric(event);'/>" +
...
function student_wise_marks_list(data) {
    ...
    if (student_marks != undefined) {
        ...
        $("#example tbody tr").each(function (i) {
            if (course_exams.length > 0) {
                //new
                $(this).children().eq(2)[0].children[0].disabled = true;
