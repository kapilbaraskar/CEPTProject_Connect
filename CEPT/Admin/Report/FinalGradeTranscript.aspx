<%@ Page Title="GradeTranscript" Language="C#" MasterPageFile="~/AdminCEPT.master" AutoEventWireup="true"
    CodeFile="FinalGradeTranscript.aspx.cs" Inherits="Admin_Report_FinalGradeTranscript" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>
                <i class="icon-desktop"></i>&nbsp;Final Transcript
            </h1>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        function display_Student_Data(data) {
            ...
            oTable = $("#example").dataTable({
                ...
                //{ "sTitle": "Course Credits", "mData": "course_credits", "bSortable": false }
                    {
                        "sTitle": "View-Transcript", "mData": null, "bSortable": false, "mRender": function (course_code)
                        {
                            return '<center><button type="button" onclick="rowClick_view(this)">View</button></center>';
                        }
                     },
                    {
                        "sTitle": "Download-Transcript", "mData": null, "bSortable": false, "mRender": function (course_code) {
                        //alert(course_code);
                        return '<center><button type="button" onclick="rowClick(this)">Download</button></center>';
                    }
                    },
                    {
                        "sTitle": "Download-Outline", "mData": null, "bSortable": false, "mRender": function (course_code) {
                            //alert(course_code);
                            return '<center><button type="button" onclick="DownloadOutline(this)">Download</button></center>';
                        }
                    },

                    {
                        "sTitle": "Download-Migration", "mData": null, "bSortable": false, "mRender": function (data) {
                            if (data.StatusFlag == 'Y') {
                                return '<center><button type="button" onclick="DownloadMigration(this)">Download</button></center>';
                            }
                            else
                            {
                                return data.StatusForApprove;
                            }
        ...
        function rowClick_view(row) {
            ...
            if (status == 'true') {
                ...
                $('#ifrm_outline').html('<iframe src="' + location.origin + '/Admin/Report/FinalTranscript.aspx?uid=' + rowId + '&sem=' + semester + '&year=' + year_code + '&new_tab=Y" width="1" height="1"></iframe>');
            }
        }

        function DownloadOutline(row) {
            var rowId = row.parentElement.parentElement.parentElement.childNodes[0].childNodes[0].nodeValue;
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "../../WebService.asmx/PrintOutlinePdf",
                async: false,
                data: "{ student : '" + rowId + "', Sem : '" + semester + "', Year : '" + year_code + "'}",
                dataType: "json",
                success: function (data) {
                    var ObjResponse = JSON.parse(data.d);
                    if (ObjResponse.Status == "1") {
                        document.getElementById('Link').download = ObjResponse.FileName;
                        document.getElementById('Link').href = window.location.origin + ObjResponse.FilePath;
                        document.getElementById('Link').click();
                    }
                    else {
                        bootbox.alert("Student does not have any passed course to display");
                    }
                },
                error: function (result) {
                    alert(result);
        ...
        function DownloadMigration(row) {
            var rowId = row.parentElement.parentElement.parentElement.childNodes[0].childNodes[0].nodeValue;
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "../../WebService.asmx/CreatePDFForMigration",
                async: false,
