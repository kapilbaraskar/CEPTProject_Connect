<%@ Page Title="GradeReport Final" Language="C#" MasterPageFile="~/AdminCEPT.master" AutoEventWireup="true"
    CodeFile="GradeReport_Final.aspx.cs" Inherits="Admin_Report_GradeReport_Final" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>
                <i class="icon-desktop"></i>&nbsp;Grade Report Final
            </h1>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        function retrieve_Student_Data() {
            ...
            $.ajax(
            {
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "../../WebService.asmx/get_gradereport_student_data",
                ...
                "aoColumns": [
...

                    {
                        "sTitle": "Select<br /><input type='checkbox' id='chk_select_all' onchange='select_all_change()' />", "mData": null, "bSortable": false, mRender: function (data) {
                            ...
                            return '<input type="checkbox" id=' + data.user_id + ' class="cls_chk_course_select" onchange="course_select_change(this)" />';
        ...
        function rowClick(row) {
            ...
            //var str_url = "http://localhost:3787/CEPT/Admin/Report/GradeReportPDF2.aspx?uid=" + rowId + "&sem=" + $('#drpsemester').val() + "&year=" + $('#drpyear').val();
        ...
        function select_all_change() {
            if ($('#chk_select_all')[0].checked) {
                ...
                var checkboxes = document.querySelectorAll('.cls_chk_course_select:checked');
                var ids = Array.from(checkboxes).map(function (checkbox) {
                    return checkbox.id;
                });
                var idsString = ids.join(',');
                document.getElementById('<%= hdnIDs.ClientID %>').value = idsString;
            }
            else {
                $('.cls_chk_course_select').removeAttr('checked');
                document.getElementById('<%= hdnIDs.ClientID %>').value = '';
            }
        }
