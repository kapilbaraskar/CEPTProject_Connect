...
function get_course_faculty_wise_feedback_report_for_send_pdf() {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../../WebService_WS.asmx/GetAllDataForSWSFeedBack",
...
function set_table_columns(row) {
    ...
    columns.push({ "sTitle": "Allocate Course", "mData": "AllocateCourse", "bSortable": false });
    columns.push({ "sTitle": "Feedback Recieved", "mData": "FeedbackRecieved", "bSortable": false });
    columns.push({
...
var pdf_file_path = "";
$(document).on("click", ".download_pdf_bulk", function (event) {
    pdf_file_path = "";
    var course_code = "";
    var instructor_code = "";
    var course_name = "";
    ...
    if (obj_selected_course.length > 0) {
        ...
        for (var i = 0; i < obj_selected_course.length; i++) {
            ...
            if (row_data['pdfstatus'] == 'Y')
            {
                status_pdf = false;

                if ($('#drpsemester').val() == 'S') {
                    semester = 'Summer';
                }
                else {
                    semester = 'Winter';
                ...
                var dept_name = row_data['dept_name'];
                var course_code = row_data['course_code'];
                var instructor_name = row_data['instructor_name'];
                var year_code = $('#drpyear').val();
                switch (dept_name) {
                    case "Architecture":
                        dep_name = "FA";
                ...
                if (pdf_file_path == "") {
                    ...
                    pdf_file_path = pdf_file_path + ',' + link;
                    //pdf_file_path = pdf_file_path.replace(' ', '_');
                }
            }
        ...
        if (status_pdf) {
            bootbox.alert("Please Generate PDF, then Download Pdf");
            return false;
        }
    }
    else {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../../WebService.asmx/WSCombinePDF_TE_Letter",
...
$(document).on("click", ".send_feedback_pdf_mail_individual_bulk", function (event) {
    ...
    if (obj_selected_course.length > 0) {
        ...
        for (var i = 0; i < obj_selected_course.length; i++) {
            ...
            publish_data.push({
                ...
                total_course: row_data['total_course'],
                total_feedback: row_data['total_feedback'],
                dept_name: row_data['dept_name'],
                instructor_name: row_data['instructor_name'],
                faculty_mail: row_data['Personal_mail_id'],
                pdfstatus: row_data['pdfstatus']
            });
        }
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../../WebService.asmx/SWSsend_feedback_pdf_mail_individual",
        data: "{'year_code':'" + year_code + "','sem_code':'" + semester + "','course_code':'" + course_code + "','course_name':'" + course_name + "','total_course':'" + total_course + "','total_feedback':'" + total_feedback + "','dept_name':'" + dept_name + "','instructor_name' :'" + instructor_name + "','mail':'" + faculty_mail + "','instructor_code':'" + instructor_code + "','bulkmail':'" + JSON.stringify(publish_data) + "'}",
        dataType: "json",
        success: function (data) {
            if (data.d != "") {
                if (data.d == "1") {
...
$(document).on("click", ".send_feedback_pdf_mail_individual", function (event) {
    ...
    var course_code = aData["course_code"];
    var instructor_code = aData["instructor_code"];
    var course_name = aData["course_name"].replace(/'/g, '');
    var total_course = aData["total_course"];
    var total_feedback = aData["total_feedback"];
    var dept_name = aData["dept_name"];
...
$(document).on("click", ".pdf_download", function (event) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../../WebService.asmx/SWSFileExits",
            ...
            if (data.d != "") {
                if (data.d == "1") {
                    ...
                    if (window.location.hostname === "localhost") {
                        window.open('http://localhost:1121/' + 'SWSFeedbackPdf' + '/' + link, '_blank');
                    }
                    else { window.open('https://connect.cept.ac.in/' + 'SWSFeedbackPdf' + '/' + link, '_blank');}
