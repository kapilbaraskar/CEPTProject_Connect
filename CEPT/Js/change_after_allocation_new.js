...
function get_allocate_data() {
...

    {
        ...
        url: "../WebService.asmx/get_saved_registerd_course_for_drop_course",
            ...
            success: function (data) {
                if (data.d != null) {
                    ...
                    if (data.d[2] != "") {
                        ...
                        saved_data = JSON.parse(data.d[2]);
                        WS_Display_saved_Data(data.d[2]);
                    }
                    else {
                        bootbox.alert('There is No data Found For Selected Student');
                        return false;
...
function Display_saved_Data(data) {
    ...
    oTable = $("#datatable_saved").dataTable({
        ...
        "aoColumns": [
              ...
              { "sTitle": "Cancel","mData": null,"sDefaultContent": '<center><input type="checkbox"  name="check1" value="1" class="chk_cancel" ></center>'}
...
function WS_Display_saved_Data(data) {
    ...
    oTable = $("#datatable_saved").dataTable({
        ...
        "aoColumns": [
            ...
            //{ "sTitle": "new_credit", "mData": "new_credit" },
            { "sTitle": "Course", "mData": "coursename", "sWidth": "500px" },
            { "sTitle": "Faculty", "mData": "dept_name" },
            { "sTitle": "Course Type", "mData": "course_type" },
            { "sTitle": "Combination", "mData": "credit_combination" },
            { "sTitle": "priority", "mData": "priority" },
            { "sTitle": "Course Credit", "mData": "credits" },
            { "sTitle": "Course Registration  Date", "mData": "RegistrationDate" },
            {
                "sTitle": "Course Drop  Date", "bSortable": false, "mData": null, fnRender: function (data) {
                    if (data.aData.course_drop_status == "Y") {
                        return data.aData.DropCourseDate
                    }
                    else {
                        return '';
                    }
            ...
            {
                "sTitle": "Drop Status", "bSortable": false, "mData": null, fnRender: function (data) {
                    if (data.aData.course_drop_status == "Y") {
                        return 'DROP'
                    }
                    else {
            ...
            {
                "sTitle": "Cancel", "bSortable": false, "mData": null, fnRender: function (data)
                {
                    if (data.aData.checkbox == "true") {
                        return '<center><input type="checkbox"  name="check1" value="1" class="chk_cancel" ></center>';
                    }
                    else {
                        return '';}
...
function save_changes() {
    ...
    if ($('.chk_cancel:checked').length > 0)
    {
    ...
    if ($('.chk_cancel:checked').length > 0)
    {
        bootbox.confirm("Are you sure to drop selected courses?", function (result)
        {
            if (result == true) {
                ...
                $("#datatable_saved tbody tr").each(function (i) {
                    var aPos = oTable.fnGetPosition(this);
                    if (aPos != null) {
                ...
                $.ajax({
                    type: "POST",
                    url: "../WebService.asmx/sws_cancel_course_data",
                    data: data,
                    contentType: "application/json; charset=utf-8",
                    datatype: "json",
                    success: function (data) {
                        ...
                        if (data.d != "") {
                            ...
                            if (data.d == "true") {
                                bootbox.alert("Course DROP Successfully");
