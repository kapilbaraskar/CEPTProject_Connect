...
$(document).ready(function () {
    ...
    bindinstructor();
    if ($('#hdnusertype').val() == 'A' || $('#hdnusertype').val() == 'FA') {
        $('.initial_pending').css('display', 'block');
        $('.cls_tabs').css('display', 'block');
    }
    if ($('#hdnusertype').val() == 'A') {
        $('.cls_tabs_rej').css('display', 'block');
    }
    $('#btnreterive').on('click', function ()
    {
        ...
        if ($('#hdnusertype').val() == 'A' || $('#hdnusertype').val() == 'FA') {
            ...
            retrieve_WS_pending_initial_course_Data();
        }
        if ($('#hdnusertype').val() == 'A' || $('#hdnusertype').val() == 'FA') {
            $('.cls_tabs').css('display', 'block');
...
function retrieve_WS_pending_initial_course_Data() {
    $('#DataList_pendinginitial').css('display', 'none');
    ...
    $.ajax(
        {
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: "../../WebService.asmx/get_ws_pending_initial_course_list",
            data: "{sem_code:'" + semester + "' , year_code : '" + year_code + "',dept_code: '" + dept_code + "'}",
            dataType: "json",
            success: function (data) {

                if (data.d != "") {
                    display_pending_initial_course_Data(data.d);
...
function display_pending_initial_course_Data(data) {

    var columns = [];

    columns = [{ "sTitle": "Course Code", "mData": "course_code", "bSortable": false },
    ...
    {
        "sTitle": "GPA Status", "mData": null, "bSortable": false, mRender: function (data) {
            if (data.gpa_status != '') {
                ...
                else if (data.gpa_status == "GN") {
                    return "GPA/NGPA (Both)";
                }
            }
            else
                return '';
    ...
    { "sTitle": "Category Location Wise", "mData": "category_location_wise", "bSortable": false },
        ...
        { "sTitle": "TA Name", "mData": "TA_instructors", "bSortable": false },
        {
            "sTitle": "Course Budget File", "mData": null, "bSortable": false, mRender: function (data) {
    ...
    {
        "sTitle": "Start Date", "mData": null, "bSortable": false, "mRender": function (data) {
            if (data.start_date != '') {
    ...
    {
        "sTitle": "End Date", "mData": null, "bSortable": false, mRender: function (data) {
            if (data.end_date != '') {
    ...
    columns.push({
        "sTitle": "Edit", "mData": null, "bSortable": false, mRender: function (data) {
            if ($("#hdnusertype").val() == 'A' || $("#hdnusertype").val() == 'FA') {
                return "<center><button type='button' onclick='edit_pending(this)' class='btn btn-primary btn-small'>Edit</button></center>";
            }
            else return "";
        }
    });
    ...
    columns.push({
        "sTitle": "View Detail", "mData": null, "bSortable": false, mRender: function (data) {
            return "<center><a onclick='view_course_detail_pending(this)'>View</button></a>";
        }
    });

    if (oTable4 != null) {
        oTable4.fnDestroy();
        $("#DataList_pendinginitial").html(' <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered table-hover" id="example_pendinginitial" width="100%"><thead></thead><tbody> </tbody></table>');
    }

    oTable4 = $("#example_pendinginitial").dataTable({
        "bPaginate": false,
        "bSortable": false,
        "bSort": false,
        "iDisplayLength": 60,
    ...
    $('#DataList_pendinginitial').css('display', 'block');
}
...
function display_offered_course_Data(data) {
    ...
    if ($("#hdnusertype").val() == 'D') {
        columns.push({ "sTitle": "Approve", "mData": null, "bSortable": false, mRender: function (data) {
            if (data.instructor_approved == 'Y') {
                return "<center><button type='button' onclick='approve(this)' class='btn btn-primary btn-small'>Approve</button></center>";
            }
            else return "";
        }
        });
        columns.push({ "sTitle": "Send for Review", "mData": null, "bSortable": false, mRender: function (data) {
            if (data.instructor_approved == 'Y') {
        ...
        columns.push({ "sTitle": "Reject", "mData": null, "bSortable": false, mRender: function (data) {
            if (data.instructor_approved == 'Y') {
        ...
        columns.push({ "sTitle": "Edit", "mData": null, "bSortable": false, mRender: function (data) {
            //if (data.instructor_approved == 'N' && data.created_by == $('#hdnuserid').val())
            //{
    ...
    columns.push({ "sTitle": "View Detail", "mData": null, "bSortable": false, mRender: function (data) {
...
//            }
    ...
    if ($("#hdnusertype").val() == 'WSA' || $("#hdnusertype").val() == 'A') {
        columns.push({
            "sTitle": "New Course Code", "mData": null, "bSortable": false, "mRender": function (data) {
                if (data.is_publish != 'Y')
                    return '<input type="text" value="' + data.ws_course_code + '" class="cls_ws_course_code" style="width:100px;" />';
                else return '' + data.ws_course_code;
            }
        });
        columns.push({
            ...
            "sTitle": "Course Type", "mData": null, "bSortable": false, "mRender": function (data) {
                if (data.is_publish != 'Y') {
                    if (data.course_type == "M") {
                        return '<select class="cls_course_type" style="width:100px;"><option value="N">Select Type</option><option value="M" selected>Mandatory</option><option value="E">Elective</option></select>';
                    } else if (data.course_type == "E") {
                        return '<select class="cls_course_type" style="width:100px;"><option value="N">Select Type</option><option value="M">Mandatory</option><option value="E" selected>Elective</option></select>';
                    } else {
                        return '<select class="cls_course_type" style="width:100px;"><option value="N">Select Type</option><option value="M">Mandatory</option><option value="E">Elective</option></select>';
                    }
                    
                } else {
                    if (data.course_type == "M") {
                        return "<span class='cls_course_type'>Mandatory</span>";
                    } else if (data.course_type == "E") {
                        return "<span class='cls_course_type'>Elective</span>";
                    } else {
                        return '';
                    }
    ...
    else
    {
        ...
        columns.push({
            "sTitle": "New Course Code", "mData": null, "bSortable": false, "mRender": function (data) {
                if (data.ws_course_code != '')
                    return '' + data.ws_course_code;
                else return '';
            }
        });
        columns.push({
    ...
    columns.push({ "sTitle": "Course Title", "mData": "course_name", "bSortable": false },
        ...
        { "sTitle": "TA Name", "mData": "TA_instructors", "bSortable": false },
        {
            "sTitle": "Edit", "mData": null, "bSortable": false, mRender: function (data) {
                ...
                {
                    return "<center><button type='button' onclick='edit_approved(this)' class='btn btn-primary btn-small'>Edit</button></center>";
                }
                else { return "";}
        ...
        //        if (data.instructor_approved == 'Y' && data.dean_approved == 'A' && ($("#hdnusertype").val() == 'A' || $("#hdnusertype").val() == 'A1')) {
        //            return "<center><button type='button' onclick='sendForReview_approved(this)' class='btn btn-primary btn-small'>Send</button></center>";
        //        }
        //        else return "";
        //    }
        //},
        ...
        $('#btn_save_course_code').on('click', function () {
            ...
            $('#example_approvedcourses tbody tr').each(function (i) {
                ...
                if ($(this).find('.cls_ws_course_code').val() != '' && $(this).find('.cls_ws_course_code').val() != undefined && $(this).find('.cls_ws_course_code').val() != row_data['ws_course_code'].toString()) {
                    var obj_changed_code_detail = { 'course_code': '', 'ws_course_code': '', 'sem_code': '', 'year_code': '', 'course_type': '' };
                    ...
                    if ($(this).find('.cls_course_type').val() == "N") {
                        obj_changed_code_detail.course_type = "";
                    } else {
                        obj_changed_code_detail.course_type = $(this).find('.cls_course_type').val();
                    }
                ...
                } else if ($(this).find('.cls_course_type').val() != "") {
                    if ($(this).find('.cls_course_type').val() != row_data['course_type'].toString()) {
                ...
                $.ajax(
                {
                    ...
                    success: function (data) {
                        if (data.d != "" && data.d != "[]") {
                            ...
                            if (res["status"].toString() == 'True') bootbox.alert("Course Codes/Course Type Saved Successfully", function () { $('#btnreterive').click() });
...
function sendForReview(element) {
    cur_course_data = {};
    $('#txt_send_mail_subject').val('SWS Course Send For Review');
    if ($("#hdnusertype").val() == 'D') {
        var row = element.closest('tr');
        var row_data = oTable.fnGetData(row);

        cur_course_data = { 'course_name': row_data['course_name'].toString().replace(/([-,.€~!@#$%^&*()_+=`{}\[\]\|\\:;'<>])+/g, ''),'course_code': row_data['course_code'].toString(), 'sem_code': semester, 'year_code': year_code, 'mail_subject': '', 'mail_body': '' };

        $('#td_send_course_name').html(row_data['course_name'].toString().replace(/([-,.€~!@#$%^&*()_+=`{}\[\]\|\\:;'<>])+/g, ''));

        $('#btn_show_modal_sendforreview').click();
    }
}

var cur_course_data_approved;
function sendForReview_approved(element) {
    cur_course_data_approved = {};

    if ($("#hdnusertype").val() == 'A' || $("#hdnusertype").val() == 'A1') {
        var row = element.closest('tr');
        var row_data = oTable1.fnGetData(row);

        cur_course_data_approved = { 'course_name': row_data['course_name'].toString().replace(/([-,.€~!@#$%^&*()_+=`{}\[\]\|\\:;'<>])+/g, ''),'course_code': row_data['course_code'].toString(), 'sem_code': semester, 'year_code': year_code, 'mail_subject': '', 'mail_body': '' };
...
function sendMail_sendforreview() {
    ...
    if ($("#hdnusertype").val() == 'A' || $("#hdnusertype").val() == 'A1') {
        ...
        cur_course_data_approved['mail_subject'] = $('#txt_send_mail_subject').val();
        cur_course_data_approved['mail_body'] = $('#txt_send_mail_body').val();
    }
    else {
    ...
    $.ajax(
    {
        ...
        //async: false,
        //(curClass == "bg_black") ? "bg_pink" : "bg_black";
        ...
        success: function (data) {
            if (data.d != "" && data.d != "[]") {
                var res = JSON.parse(data.d);
                if (res["status"]) {
                    bootbox.alert("Course Send for Review Successfully", function () {
...
function reject(element) {
    ...
    $('#txt_reject_mail_subject').val('SWS Course Reject');
    if ($("#hdnusertype").val() == 'D') {
        var row = element.closest('tr');
        var row_data = oTable.fnGetData(row);
        ...
        $('#td_reject_course_name').html(row_data['course_name'].toString().replace(/([-,.€~!@#$%^&*()_+=`{}\[\]\|\\:;'<>])+/g, ''));
...
function edit(element) {
    ...
    if ($("#hdnusertype").val() == 'I2' || $("#hdnusertype").val() == 'PC' || $("#hdnusertype").val() == 'D' || $("#hdnusertype").val() == 'WSA' || $("#hdnusertype").val() == 'A' || $("#hdnusertype").val() == 'FA')
    {
        var row = element.closest('tr');
        var row_data = oTable.fnGetData(row);
        //window.location = "ws_coursemaster_add.aspx?c=" + row_data['course_code'] + "&s=" + semester + "&y=" + year_code;
        window.open("ws_coursemaster_add.aspx?c=" + row_data['course_code'] + "&s=" + semester + "&y=" + year_code, '_blank'); // as per request by nitin bhai 22092023
    }
}

function edit_approved(element) {
    
    if ($("#hdnusertype").val() == 'I2' || $("#hdnusertype").val() == 'PC' || $("#hdnusertype").val() == 'D' || $("#hdnusertype").val() == 'WSA' || $("#hdnusertype").val() == 'A' || $("#hdnusertype").val() == 'FA') {
        var row = element.closest('tr');
        var row_data = oTable1.fnGetData(row);
        var course_code_new = '';
        if (row_data['ws_course_code'].trim() != '') {
            course_code_new = row_data['ws_course_code'].trim();
        }
        else { course_code_new = row_data['course_code'].trim(); }
        window.open("ws_coursemaster_add.aspx?c=" + row_data['course_code'] + "&s=" + semester + "&y=" + year_code, '_blank'); // as per request by nitin bhai 22092023
        //window.location = "ws_coursemaster_add.aspx?c=" + row_data['course_code'] + "&s=" + semester + "&y=" + year_code;
    }
}

function edit_pending(element) {

    if ($("#hdnusertype").val() == 'A' || $("#hdnusertype").val() == 'FA') {
        var row = element.closest('tr');
        var row_data = oTable4.fnGetData(row);
        //window.location = "ws_coursemaster_add.aspx?c=" + row_data['course_code'] + "&s=" + semester + "&y=" + year_code;
        window.open("ws_coursemaster_add.aspx?c=" + row_data['course_code'] + "&s=" + semester + "&y=" + year_code, '_blank'); // as per request by nitin bhai 22092023
    }
}
...
function view_course_detail_pending(element) {
    var row = element.closest('tr');
    var row_data = oTable4.fnGetData(row);
...
//                case "5": $('#td_view_inhabitation').html("Technology"); break;
    //            }
    if (row_data != null) {
        $('#spn_course_code').html('<b class="cls_colon">:</b>' + row_data["course_code"]);
        ...
        if (row_data["gpa_status"] == "G") {
            $('#spn_gpa_status').html('<b class="cls_colon">:</b>GPA');
        }
        else if (row_data["gpa_status"] == "N")
        {
            $('#spn_gpa_status').html('<b class="cls_colon">:</b>NGPA');
        }
        else if (row_data["gpa_status"] == "GN") {
            $('#spn_gpa_status').html('<b class="cls_colon">:</b>GPA/NGPA (Both)');
        }
...
//                if (row_data["course_output3"].toString() != '') $('#ul_course_output').append('<li>' + row_data["course_output3"] + '</li>');
        ...
        if (row_data["learning_outcomes"] != '') {
            $('#div_learning_out').html(row_data["learning_outcomes"]);
        }
    }
...
function set_course_instructor_workplan_detail(course_code) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var str_category_location_wise = '';
            if (data.d[0] != null) {
                var ws_course_data = JSON.parse(data.d[0]);
                str_category_location_wise = ws_course_data[0]['category_location_wise'];
            }
            ...
            if (data.d[1] != null) {
                ...
                for (var i = 0; i < course_instructor_data.length; i++) {
                    ...
                    $('#phead_desc_faculty' + (i + 1)).html($('#drp_instructor_list')[0].selectedOptions[0].innerHTML.trim());
                    $('#lbl_desc_faculty' + (i + 1)).html($('#drp_instructor_list')[0].selectedOptions[0].innerHTML.trim());
                    ...
                    else if (str_category_location_wise == 'Travel Based Outside India' || str_category_location_wise == 'Travel Based Within India') {
                        $('.cls_travel_accomodation' + (i + 1)).css('display', 'block');
                    }
            ...
            if (data.d[2] != null) {
                ...
                for (var i = 0; i < course_workplan_data.length; i++) {
                    ...
                    for (var j = 0; j < course_workplan_data[i]["faculty_involve"].split('~').length; j++) {
                        $('#drp_instructor_list').val(course_workplan_data[i]["faculty_involve"].split('~')[j]);

                        if (j != 0) str_faculty_involve += ' , ';
                        
                        str_faculty_involve += $('#drp_instructor_list')[0].selectedOptions[0].innerHTML.trim();
                    }
                    ...
                    var str_tbody = '<tr id="' + temp_date + '"><td>Day' + (i + 1) + '</td>' +
                            ...
                            '<td style="display:none;">' + course_workplan_data[i]["topic_covered"] + '</td>' +
                            '<td>' + course_workplan_data[i]["methodology"] + '</td>' +
                            '<td><span class="cls_total_hrs">' + course_workplan_data[i]["no_of_hrs"] + '</span></td>' +
                            '<td><span class="cls_faculty_involve">' + str_faculty_involve + '</span></td></tr>';
    ...
    if ($('#tbl_workplan tbody tr').length > 0) {
        ...
        $('#tbl_workplan tbody tr').each(function (i) {
            if ($(this).find('.cls_faculty_involve').html() != '' && parseFloat($(this).find('.cls_total_hrs').html()).toString() != 'NaN') {
                ...
                var obj_faculty_involve = $(this).find('.cls_faculty_involve').html().split(',');

                for (var j = 0; j < obj_faculty_involve.length; j++) {
                    options_total_workload[obj_faculty_involve[j].trim()] += parseFloat($(this).find('.cls_total_hrs').html());

                    options_total_workload[obj_faculty_involve[j].trim()] = parseFloat(options_total_workload[obj_faculty_involve[j].trim()].toFixed(2));

                    if (options_total_workload[obj_faculty_involve[j].trim()].toString().split('.').length == 2) {
                        if (parseFloat(options_total_workload[obj_faculty_involve[j].trim()].toString().split('.')[1]) == 6) {
                            options_total_workload[obj_faculty_involve[j].trim()] += 0.40;
                        }
                    }
                }
            }
