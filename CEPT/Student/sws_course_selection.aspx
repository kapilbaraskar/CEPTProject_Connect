<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPageDesign.master" AutoEventWireup="true" CodeFile="sws_course_selection.aspx.cs" Inherits="Student_sws_course_selection" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('#btnsave').on('click', function ()
            {
                ...
                for (var q = 1; q <= course_combination; q++)
                {
                    ...
                    if (course_count.length > 2) {
                        ...
                        if (found_count.length < 3) {
                            bootbox.alert("Minimum 3 & Maximum 8 Preferences to complete the SW Course Registration for each course choice");
                            return false;
                ...
                //if (datalist.length != count_course)
                //{
                //    bootbox.alert("Minimum 3 & Maximum 8 Preferences to complete the SW Course Registration for each course choice");
                //    return false;
                //}
                ...
                //    bootbox.alert("MAX 3 Course Select");
                //    return false;
                //}
                //return false;
        ...
        function get_course_dtl() {
            ...
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "../../WebService.asmx/Get_SWS_Course_dtl",
                data: "{}",
                dataType: "json",
                async: false,
                success: function (data) {
                    ...
                    if (data.d[1] != "" && data.d[1] != null) {
                        ...
                        for (var i = 0; i < reg_data.length; i++)
                        {
                            ...
                            //$('#cls_chk_course_select_' + reg_data[i]['course_code'] + '_' + reg_data[i]['credit_combination']).attr('checked', 'checked');
        ...
        function display_student_Course(data) {
            ...
            prio_option += '<option value="4">4</option>';
            prio_option += '<option value="5">5</option>';
            prio_option += '<option value="6">6</option>';
            prio_option += '<option value="7">7</option>';
            prio_option += '<option value="8">8</option>';
            //prio_option += '<option value="9">9</option>';
            //prio_option += '<option value="10">10</option>';
            //prio_option += '<option value="11">11</option>';
            //prio_option += '<option value="12">12</option>';
            //prio_option += '<option value="13">13</option>';
            //prio_option += '<option value="14">14</option>';
            //prio_option += '<option value="15">15</option>';
            if (oTable != null) {
                oTable.fnDestroy();
            ...
            oTable = $("#example").dataTable({
                ...
                "aoColumns": [
                    ...
                    //        return '<input type="checkbox" id="cls_chk_course_select_' + data.course_code + '_' + data.credit_combination + '" class="cls_chk_course_select" onchange="course_select_change(this)"/>';
