...
public class proposal : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        try
        {
            ...
            string Serverpath = System.Configuration.ConfigurationManager.AppSettings["projectProposalPath"];

            if (context.Request.Files.Count > 0)
            {
                var postedFile = context.Request.Files[0];
                string filesize = System.Configuration.ConfigurationManager.AppSettings["projectProposalMaxSize"];
