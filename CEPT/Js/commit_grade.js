...
function rowClick_commit(row) {
    
    var rowId = row.parentElement.parentElement.parentElement.childNodes[1].childNodes[0].nodeValue;

    var url = "Calculated_Commit_Grade.aspx?c=" + rowId + "&s=" + semester + "&y=" + year_code;
...
function commit_all() {
    ...
    if ($("#example_filter :input").val() != '') {
        bootbox.alert("Please Remove Course Code in Search Box then Commit Grade.");
        return false;
    }

    if ($('#hdnusertype').val() == 'A') {
        bootbox.confirm('Are you sure you want to Commit Grade for the Selected courses?', function (result) {
            if (result == true) {
                                ...
                                if (obj_selected_course.length > 0) {
                                    ...
                                    for (var i = 0; i < obj_selected_course.length; i++) {
                                        ...
                                        //if (row_data['ugpg_approval'] != 'Approved') {
                                            commit_data.push(row_data['course_code']);
                                        //}
                                    }
                                    ...
                                    if (commit_data.length > 0) {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            contentType: "application/json; charset=utf-8",
                                            url: "../../WebService.asmx/commit_selected_course_grade",
                                            //async: false,
                                            data: "{commit_data:'" + str_commit_data + "',sem_code:'" + semester + "',year_code:'" + year_code + "'}",
                                            dataType: "json",
                                            success: function (data) {
                                                if (data.d != "" && data.d != "[]") {
                                                    if ($('#hdnusertype').val() == 'A') {
                                                        var res = data.d;
                                                        if (res == 'Grade Commited Successfully.') {
                                                            bootbox.alert("Course Committed Successfully", function () {
                                                                $('#btnreterive').click();
                                                            });
                                                        }
                                                        else
                                        ...
                                        bootbox.alert('No Courses Selected to Commit Grade');
                                    }
                                }
                                else {
                                    bootbox.alert('No Courses Selected to Commit Grade');
        ...
        bootbox.alert('You are not Authorized to Commit Course Grade');
    }
}
    ...
    $.ajax(
        {
            ...
            success: function (data) {
                if (data.d != "" && data.d != "[]") {
                    if ($('#hdnusertype').val() == 'A') {
                        course_wise_entered_marks_list_A(data.d);//A1-course_wise_entered_marks_list_UGPG
                    }
                    else if ($('#hdnusertype').val() == 'FA') {
...

            {
                "sTitle": "", "mData": null, "bSortable": false, fnRender: function (data) {
                    if ($('#hdnusertype').val() == 'I2' || $('#hdnusertype').val() == 'PC' || $('#hdnusertype').val() == 'D') {
                        if (data.aData.faculty_approval != 'Approved' && data.aData.progcoordinate_approval != 'Approved' && data.aData.ugpg_approval != 'Approved') {
                            return '<center><button type="button" onclick="rowClick(this)">Edit</button></center>';
                        }
                        else {
...
function course_wise_entered_marks_list_FA(data) {
    ...
    oTable = $("#example").dataTable({
        ...
        "aoColumns": [
            {
                "sTitle": "Select<br /><input type='checkbox' id='chk_select_all' onchange='select_all_change()' />", "mData": null, "sClass": "hide", "bSortable": false, fnRender: function (data) {
            ...
            {
                "sTitle": "Grade", "mData": null, "bSortable": false, fnRender: function (data) {//Commit Grade
                        //if (data.aData.faculty_approval != 'Approved' || data.aData.progcoordinate_approval != 'Approved') {
                        return '<center><button type="button" onclick="rowClick_commit(this)">Verify</button></center>';
                        //}
                        //else {
                        //    return '<center><button type="button" onclick="rowClick_view(this)">View</button></center>';
                        //}
    ...
    //$('#submitBtnDiv').html('<button id="btn_publish_all" type="button" class="btn btn-lg btn-primary" onclick="commit_all()">Commit All</button>');

}

function course_wise_entered_marks_list_A(data) {
    ...
    oTable = $("#example").dataTable({
        ...
        "aoColumns": [
            {
                "sTitle": "Select<br /><input type='checkbox' id='chk_select_all' onchange='select_all_change()' />", "mData": null, "sClass": "", "bSortable": false, fnRender: function (data) {
                    //if ($('#hdnusertype').val() == 'AD') {
                    //    if (data.aData.ugpg_approval != 'Approved') {
                    //        return '<input type="checkbox" class="cls_chk_course_select" onchange="course_select_change(this)" />';
                    //    }
...

            {
                "sTitle": "Grade", "mData": null, "bSortable": false, fnRender: function (data) {//Commit Grade
                    //if (data.aData.faculty_approval != 'Approved' || data.aData.progcoordinate_approval != 'Approved') {
                    return '<center><button type="button" onclick="rowClick_commit(this)">Commit</button></center>';
                    //}
                    //else {
                    //    return '<center><button type="button" onclick="rowClick_view(this)">View</button></center>';
                    //}
