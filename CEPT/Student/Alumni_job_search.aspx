<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPageDesign.master" AutoEventWireup="true"
    CodeFile="Alumni_job_search.aspx.cs" Inherits="Student_Alumni_job_search" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div class="row-fluid">
        ...
        <div class="panel panel-default ">
            <div class="row">
                <div class="col-md-2" style="font-size: 20px; margin-top: 15px; margin-left: 25px;">
                    <input type="text" placeholder="Keyword" id="txt_keyword" />
                </div>
                <div class="col-md-2" style="font-size: 20px; margin-top: 15px; margin-left: 20px;">
            ...
            <div style="margin-left: 20px;" id="search_job">
            </div>
        </div>
        <div>
    ...
    <script type="text/javascript">
        ...
        function get_alumni_data(no) {
            ...
            //    }
            //});
            window.location.href = "Alumni_View_job.aspx?doc_no=" + no;
        }
                    ...
                    if (data.d != "") {
                        ...
                        if (response['status'] == 'true') {
                            ...
                            for (var i = 0; i < response["message"].length; i++) {
                                ...
                                str += ' <a  onclick=get_alumni_data(' + response["message"][i]["Doc_no"] + ')><image src="http://alumni.cept.ac.in/image/' + response["message"][i]["image_path"] + '" style="width: 100px;"></image></a> ';
                                str += '</div> ';
                                str += ' <div class="col-lg-2"> ';
                                //str += '   <b><span>' + response["message"][i]["job_title"] + '</span> </b>';
                                str += ' <a style="cursor: pointer;" onclick=get_alumni_data(' + response["message"][i]["Doc_no"] + ')>  <b><span>' + response["message"][i]["job_title"] + '</span> </b></a>';
                                str += ' <br /> ';
                                str += '  <span>' + response["message"][i]["organization"] + '</span> ';
                                str += ' </div> ';
                                str += ' <div class="col-lg-2"> ';
                                str += '  <span>' + response["message"][i]["state"] + ',' + response["message"][i]["city"] + '</span><br /> ';
                                str += ' <span>' + response["message"][i]["experience"] + '</span> <br/>';
                                if (response["message"][i]["is_immediately"] == "Y") {
                                    str += '  <span>immediately</span> ';
                                } else {

                                    str += '  <span>' + response["message"][i]["due_date"] + '</span> ';
                                }
                                str += ' </div> ';
                                str += '<div class="col-lg-2"> <span style="color:#08C">Views <span class="badge"> ' + response["message"][i]["jobview"] + '</span></span> </div>';
                                str += '</div> ';
        ...
        function PageLogActivite() {

            var obj_data = {};
            obj_data.jobid = "";
            obj_data.pagename = "AlumniSearchJob.aspx";

            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "../WebService.asmx/Save_jobPageActiviteLog",
                async: false,
                data: "{str_req_data:'" + JSON.stringify(obj_data) + "'}",
                dataType: "json",
                success: function (data) {
