...
$(document).ready(function () {
    ...
    type_per = getParameterByName('per');
    per_course_code = getParameterByName('icc');
    $("#sd").css('display', 'none');

    if (type == "tutor")
    {
        $('#tutor_hide').css("display", "none");
        $('#tutor_portfolio_hide').css("display", "none");
        ...
        if ($('#hdn_icode_ex').val() != "") {
            ...
            else
            {
                str = "<table style='width: 100%'><tr><td align='center' style='width: 40%;'> "
                    + "<button id = 'btnsave' type = 'button' style='display:block; margin-right: -75%;' class='btn btn-primary'>"
                    + "<i class='icon-save bigger-160' ></i> Save</button ></td > <td align='' style='width: 20%;'> <button id = 'btnapprove' type = 'button' style='display:block; float: center;' class='btn btn-primary'>"
                    + "Step 2 >></button></td><td align='center' style='width: 40%;visibility: collapse;'><button id = 'btnnext' type = 'button' style='margin-left:-136%;' class='btn btn-primary'>"
                    + " Step 2 >></button ></td ></tr ></table > ";
            }
    ...
    $('#btnapprove').on('click', function () {
        ...
        if (!user_status) {
            ...
            var work_experience_certificate = false;
            $('#tbl_work_experiance tbody tr').each(function (i) {
                if (i > 0) {
                    ...
                    if (file_name_class != undefined) {
                        ...
                        if ($('.' + file_name_class).text() == '') {
                            work_experience_certificate = true;
                           // return false;
                        }
                        else { work_row.work_ex_cert = $('.' + file_name_class).text();}
                    }
                    else {
                        work_row.work_ex_cert = '';
                        work_experience_certificate = true;
                        return false;
                    }
            ...
            //if (work_experience_certificate)
            //{
            //    bootbox.alert('Please Upload WorkExperience Certificate ');
            //    return false;
            //}
        ...
        else if (type == "tutor")
        {
            ...
            if (messagehandle !== "")
            {
                if (messagehandle == "Aadhar No Already exists Please Change Aadhar No")
                {
                    //alert(messagehandle);
                    return false;
                }
                else if (messagehandle == "PAN No Already exists Please Change PAN No") {
                    //alert(messagehandle);
                    return false;
                }
                alert(messagehandle); // Log or handle the message if necessary
            }
            if (!user_status)
            {
               // alert('Personal Details Submitted Successfully.');
    ...
    $('#btnsave').on('click', function () {
        ...
        var instructor_data = {
            ...
            'total_work_experience_year': '', 'total_work_experience_month': '', 'social_security_no': '', 'online_working_status': ''
        };
        ...
        var personal_detl_array = [];
        var personal_detl_status = { 'personal_detl_status': '', 'per_course_code': '' };
        if (type != "" && type_per != "")
        {
            personal_detl_status.personal_detl_status = 'P';
            personal_detl_status.per_course_code = per_course_code;
            personal_detl_array.push(personal_detl_status);
        }
        else {
            personal_detl_status.personal_detl_status = '';
            personal_detl_status.per_course_code = '';
            personal_detl_array.push(personal_detl_status);
        }

        var All_instructor_data = [instructor_data, reference_dtl, education_dtl, work_dtl, action, personal_detl_array];
        ...
        $.ajax({
            ...
            success: function (data) {
                    ...
                    if ($('#hdnusertype').val() == 'FA' || $('#hdnusertype').val() == 'I2' || $('#hdnusertype').val() == 'PC') {
                        if (action == 'A')
                        {
                            bootbox.alert('Personal Details Submitted Successfully.', function ()
                            {
                                ...
                                if (type_per.toLowerCase() == 'p')
                                {
                                    var url = "Course_information_tabular.aspx";
                                    window.open(url, "_self");
                                }
                                else
                                { location.reload(); }
                ...
                else if (data.d == 'Aadhar No Not exists') {
                    alert('Aadhar No Not exists');
                    return false;
                }
                else if (data.d == 'PAN No Not exists') {
                    alert('PAN No Not exists');
                    return false;

                }
                else if (data.d != "")
                {
                    messagehandle = data.d;
...
function add_row(tbl) {
    if (tbl == 'reference') {
        ...
        if (designation_type.toUpperCase().trim() == "INSTRUCTOR") {
            $('.experienceupload').closest('.file-upload').css('display', 'none');
            $('.workexperiencehide').css('display', 'none');
            $('span[class^="lbl_experience_file_name_"]').css('display', 'none');
        }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.d[0] != null && data.d[0] != '') {
                ...
                //<a href="../InstructorCVUpload/something.pdf">
                FileName = instructor_data[0]["cv_file_name"];
                //InstructorPortfolioUpload
                ...
                //13092021
                $('#lbl_passport_file_name').html('<b>' + instructor_data[0]["passport_doc"] + '</b>');
                    ...
                    for (var i = 0; i < instructor_education_work.length; i++) {
                        if (instructor_education_work[i]['detail_type'] == 'reference') {
                            ...
                            var str_row = "<tr><td><input type='text' class='marg-btm' value='' style='width: 180px;'/></td><td><input type='text' class='marg-btm' value='' style='width: 120px;'/></td><td><select class='experience_type' style='width: 140px;'>"
                                ...
                                + "<input type = 'file' name = 'experienceupload' class='experienceupload' id = 'experienceupload_" + table_length + "' onchange = 'javascript: return UploadWorkExperienceCertificate(this);' /></label> "
                                + "<span class='lbl_experience_file_name_" + table_length + "'></span></td>"
                                + "<td><i class='icon-trash icon-2x text-blue' style='cursor: pointer;'></i></td> "
                                + "</tr> ";
            ...
            if (instructor_data[0]["designation"].toUpperCase().trim() == "INSTRUCTOR") {
                designation_type = instructor_data[0]["designation"].toUpperCase().trim();
                $('.experienceupload').closest('.file-upload').css('display', 'none');
                $('.workexperiencehide').css('display', 'none');
                $('span[class^="lbl_experience_file_name_"]').css('display', 'none');
            }
