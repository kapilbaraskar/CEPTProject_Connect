<%@ Page Title="" Language="C#" MasterPageFile="~/AdminCEPT.master" AutoEventWireup="true" CodeFile="Studio_Proposal_dtl_new.aspx.cs" Inherits="Admin_Master_Studio_Proposal_dtl_new" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('#btnexcel').on('click', function () {
                ...
                $('#hdn_dept').val($('#drpdepartment').val());
                $('#hdn_prog').val($('#drpprog').val());
                $('#hdn_type').val($('#drptype').val());
                $('#btn_approved').click(); 
        ...
        function bindtypedata() {
            ...
            $('#drptype').append($("<option></option>").val("N").html("Proposal Saved"));
            $('#drptype').append($("<option></option>").val("Y").html("Proposal Submitted"));
        ...
        function get_studio_detail() {
           ...
           $('#example').dataTable().fnDestroy();
            url_dtl = '../../WebService.asmx/Get_instructor_wise_shortlist_detail_v1';
           non_shortlist = true;
            ...
            $.ajax(
                {
                    ...
                    contentType: "application/json; charset=utf-8",
                    url: url_dtl,
                    data: "{sem_code:'" + semester + "',year_code:'" + year_code + "',prog_code:'" + prog_code + "',dept_code:'" + dept_code_ + "',type:'" + type + "'}",
                    dataType: "json",
                    success: function (data) {
            ...
            var columns = [
...

                {
                    "sTitle": "Reject", "mData": null, "bSortable": false, mRender: function (data) {
                        if (data.approved == 'Y') {
                            var row_value = data.user_id + '_' + data.studio_code;
                            return '<center><button type="button" id=' + row_value + ' onclick="rowClick_reject(this)" disabled>Reject</button></center>';

                        }
                        else if (data.shortlist_status == "" || data.shortlist_status == "A") {

                            var row_value = data.user_id + '_' + data.studio_code;
                            return '<center><button type="button" id=' + row_value + ' onclick="rowClick_reject(this)">Reject</button></center>';
                        }
                        else if (data.shortlist_status == "R") {//
                            return 'Rejected';
                        }
                        else return '';
                    }
        ...
        function display_studio_proposal_detail(data) {
            ...
            for (var i = 0; i < data_dtl.length; i++) {
                ...
                if (status_bind) {
                    ...
                    if (data_dtl[i]['course_code'] != "" && data_dtl[i]['course_code'] != null) {
                        str += "<td><center><button type='Button' onclick='ViewBrief(" + '"' + data_dtl[i]['course_code'] + '"' + ")'>View</button></center></td>";
                    }
                    else {
                        str += "<td></td>";
                    }
                   // str += "<td><textarea id='" + data_dtl[i]['studio_code'] + "' name='remark' rows='4' cols='50'>" + data_dtl[i]['remark'] + "</textarea></td>";
                    ...
                    //    if (data_dtl[i]['approved'] == 'Y') {
                    //        var row_value = data_dtl[i]['user_id'] + '_' + data_dtl[i]['studio_code'];
                    //        str += '<td><center><button type="button" id=' + row_value + ' onclick="rowClick_reject(this)" disabled>Reject</button></center></td>';

                    //    }
                    //    else if (data_dtl[i]['shortlist_status'] == "" || data_dtl[i]['shortlist_status'] == "A") {

                    //        var row_value = data_dtl[i]['user_id'] + '_' + data_dtl[i]['studio_code'];
                    //        str += '<td><center><button type="button" id=' + row_value + ' onclick="rowClick_reject(this)">Reject</button></center></td>';
                    //    }
                    //    else if (data_dtl[i]['shortlist_status'] == "R") {//
                    //        str += '<td>Rejected</td>';
                    //    }
                    //    else str += '<td></td>';
                    //}
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
                    ...
                    <div>
                        <table border="0" cellpadding="10" cellspacing="5">
                            ...
                            <tr>
                                ...
                                <td>
                                     <button class="btn  btn-primary" type="button" id="btnexcel"> Download Excel</button>
                                </td>
                                <%--<td>Program Level :
                                </td>
                                <td>
    ...
    <div id="DataList" style="display: none; overflow: auto;" class="panel panel-default">
                <div class="panel-heading">
                    <strong id="panel_head"> Studio Proposal Details</strong>
                </div>
    ...
    <input type="hidden" id="hdn_type" runat="server" clientidmode="Static" />
