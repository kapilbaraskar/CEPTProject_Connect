<%@ Page Language="C#" AutoEventWireup="true" CodeFile="StudioUnits_SeatAllocStatus.aspx.cs" Inherits="Student_StudioUnits_SeatAllocStatus" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            temp_interval = setInterval(function () { get_available_seat_data(); }, 10000);
        });
                    ...
                    if (data.d != "") {
                        ...
                        if (course_data['message'] != '') {
                            ...
                            for (var i = 0; i < obj_data.length; i++) {
                                ...
                                str_html += '<td style="font-size:0.7em;">' + obj_data[i]['short_list_stu'] + '</td>';
                                //str_html += '<td>' + obj_data[i]['course_name'] + '</td>';
                                //str_html += '<td>' + obj_data[i]['instructor_name'] + '</td>';
                                str_html += '<td style="font-size:0.7em;">' + obj_data[i]['allocate_stu'] + '</td>';
                                ...
                                for (var e = 0; e < count_priority.length; e++)
                                {
                                    ...
                                    {
                                        var total = "";
                                        if (parseInt(allocate_sets) <= parseInt(count_priority[e]["count_priority"]))
                                        { total = parseInt(count_priority[e]["count_priority"]) - parseInt(allocate_sets); }
                                        else
                                        {
                                            total = parseInt(allocate_sets) - parseInt(count_priority[e]["count_priority"]);
                                        }
                                        
                                        str_html += '<td class="ps" style="text-align:center;    padding: 2px !important; border:0px !important;width:16.66% !important;">' + total + '</td>';
                                    }
                                    else
                                    {
