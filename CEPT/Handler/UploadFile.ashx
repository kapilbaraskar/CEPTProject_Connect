        ...
        try
        {
            ...
            else
            {
                ...
                if (mFileSize <= Convert.ToInt32(filesize))
                {
                    ...
                    if (context.Request.Form["UploadType"] == "Student")
                    {
                        message = web.ExcelFileToDatatable_uplad(file, context);
                    }
                    else
                    { 
                        message = web.ExcelFileToDatatable_upload_course_master(file, context, context.Request.Form["session_data"], context.Request.Form["course_code"], context.Request.Form["semester_type"], context.Request.Form["year_code"]);    
                    }
