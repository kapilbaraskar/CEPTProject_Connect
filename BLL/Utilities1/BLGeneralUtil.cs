...
namespace BLL.Utilities1
{
    public class BLGeneralUtil
    {
        ...
        {
            //Log.PaymentTransactionLog("IN updatetable");
            UpdateTableInfo structUpdateTableInfo;
            try
            {
                #region Initial Validations
                ...
                foreach (DataRow drow in dt.Rows)
                {
                    ...
                    switch (drow.RowState)
                    {
                        ...
                        case DataRowState.Added:
                            ...
                            SQLDelete = "DELETE FROM " + dt.TableName +
                                        " WHERE " + WhereColumns;

                            //Log.PaymentTransactionLog("IN updatetable" + SQLDelete);
                            ...
                            //Log.PaymentTransactionLog("IN updatetable" + SQL);
                            //MessageBox.Show(SQL, "INSERT");
                            break;
                        #endregion
                    ...
                    if (IsChangeFound)
                    {
                        if (updateMethod == UpdateMethod.DeleteAndInsert)
                        {
                            if (SQLDelete.Trim() != "")
                            {
                                ...
                                Log.PaymentTransactionLog("In delete and INsert " + SQLDelete);
                                int SqlTest = comm.ExecuteNonQuery();
                                //if (comm.ExecuteNonQuery() != 1)
                                //System.Windows.Forms.MessageBox.Show(muck.ToString());
                                if (SqlTest != 1 && SqlTest != 0)
                                {
                                    Log.PaymentTransactionLog("Database Failed " + SQLDelete);
                        ...
                        comm.CommandType = CommandType.Text;
                        comm.CommandText = SQL;
                        Log.PaymentTransactionLog("before Execute " + SQL);
                        if (comm.ExecuteNonQuery() != 1)
                        {
                            Log.PaymentTransactionLog("Data base failed");
                            structUpdateTableInfo.ErrorMessage = "Database updation failed.";
                            structUpdateTableInfo.ErrorSQLStatement = SQL;
                            structUpdateTableInfo.ErrorRow = drow;
                            structUpdateTableInfo.Status = false;
