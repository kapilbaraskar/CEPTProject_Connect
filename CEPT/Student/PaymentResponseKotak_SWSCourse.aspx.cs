...
public partial class Student_PaymentResponseKotak_SWSCourse : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            Log.PaymentTransactionLog("SWSCourse Fees Payment Response From Kotak. " + encResponse);
            ...
            if (!IsPostBack)
            {
                ...
                try
                {
                    ...
                    {
                        Log.PaymentTransactionLog("SWSCourse Fees Payment transaction id on cancel payment  : " + Params["order_status"]);
                    }
                    ...
                    if (UID != null)
                    {
                        ...
                        string message = objPayment.SaveSWSCoursePaymentDetails_Kotak(Params, ref paymentResponse, Params["merchant_param3"]);

                        if (paymentResponse.status.Contains("paymentSuccess"))
                        {
                            Masters objGetMasterDetails = new Masters();

                            Log.PaymentTransactionLog("SWSCourse Fees Payment transaction id : " + paymentResponse.data.transaction_id);
                            Log.PaymentTransactionLog("SWSCourse Fees Payment User id : " + paymentResponse.data.user_id);

                            string userid = paymentResponse.data.user_id.ToString();
                            string transactionid = paymentResponse.data.transaction_id.ToString();
                            // This Code already Exist In Payment Transction
                            // bool status;
                            //try
                            //{
                            //    status = objGetMasterDetails.sws_course_allocation_dtl(userid, transactionid);
                            //    Log.PaymentTransactionLog("Update Status : " + status);
                            //}
                            //catch (Exception ex)
                            //{
                            //
                            //    Log.PaymentTransactionLog("Update Status : " + ex.Message);
                            //}
                            ...
                            if (paymentResponse.data.user_id != null && paymentResponse.data.user_id != string.Empty)
                            {
                                ...
                                if (dtCandidatePersonalDetails != null && dtCandidatePersonalDetails.Rows.Count > 0)
                                {
                                    ...
                                    try
                                    {
                                        DataTable dt_transaction_data = objGetMasterDetails.get_swscourseTransactionsDetail_for_mail_send(paymentResponse.data.transaction_id);
                                        ...
                                        string mail_status = objmail.SendEmailOnSWSCoursefeesPaymentSuccess(email_data);
                            ...
                            Log.PaymentTransactionLog("Insert Data start: " + paymentResponse.data.user_id);
                            
                            //WebService obj_service = new WebService();
                            //obj_service.sws_course_allocation_dtl();
                            
                            
                            Log.PaymentTransactionLog("Insert Data End: " + paymentResponse.data.user_id);
                ...
                try
                {
                    ...
                    if (UID != null && paymentResponse.status == "paymentFail" && Request["ReferenceNo"] != null)
                    {
                        ...
                        DataTable dt_transaction_data = objGetMasterDetails.get_swscourseTransactionsDetail_for_mail_send(Request["ReferenceNo"].ToString());
                        ...
                        string mail_status = objmail.SendEmailOnSWSCoursefeesPaymentFail(dic_payment_fail_dtl);
