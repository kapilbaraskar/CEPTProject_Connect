<%@ WebHandler Language="C#" Class="Upload_File_Course_Budget" %>
...
using System.IO;
using System.Data;
public class Upload_File_Course_Budget : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        try
        {
            ...
            else
            {
                ...
                string Serverpath = System.Configuration.ConfigurationManager.AppSettings["CourseBudget"];
                ...
                DataTable dt = objMaster.Get_cept_current_sem_data("ws_course");
                if (mFileSize <= Convert.ToInt32(filesize))
                {
                    ...
                    else
                    {
                        ...
                        //string path = DateTime.Today.ToString("yyyyMMdd") + DateTime.Now.ToString("_hhmmss_");
                        string path = dt.Rows[0]["sem_code"].ToString() + dt.Rows[0]["year_code"].ToString() +'_'+ new_number + "_CourseBudget.xlsx";

                        //file = path + "_" + postedFile.FileName;
                        //file = UploadType + "_" + path + "_" + postedFile.FileName;
                        file = UploadType + "_" + path ;
                    }
                    ...
                    //if (UploadType == "userid_document")
                    //{
                    //    message = web.ExcelFileToDatatable_userid_document(file, context);
                    //}
                    //else
                    //{
                    //    message = web.ExcelFileToDatatable_scholarship_data(file, context);
                    //}
