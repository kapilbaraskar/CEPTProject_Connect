<%@ Page Title="Acceptance" Language="C#" MasterPageFile="~/AdminCEPT.master" AutoEventWireup="true" CodeFile="VF_Acceptance.aspx.cs" Inherits="Admin_Master_VF_Acceptance" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    ...
    <div class="well" style="background-color: White;">
        ...
        <div id="submitBtnDiv" style="text-align: center;margin:10px;display:none;">
            <input type="button" id="btn_accept" value="Save" class="btn btn-primary" style="display:none;"/>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        function get_authorize_detail() {
            ...
            $.ajax(
            {
                ...
                url: "../../WebService.asmx/get_authorize_detail_vf_acceptance_new",
                //async: false,
        ...
        function display_get_vf_personal_detail(data) {
            ...
            var columns = [
...

                {
                    "sTitle": "Date", "mData": null, "bSortable": false, mRender: function (data) {
                        if (data.acceptance == 'Y') {
                            return "<span style=color:blue;>Already Accepted</span>";
                        }
                        else if (data.acceptance != 'Y') {

                            return "<button id='" + data.instructor_code + "' type='button' onclick=updatedata(this) class='btn btn-primary'>Save</button>";
                        }
                        else { return "Please Connect to Admin";}
                    }
                }
                    ];
   ...
   //             },
            ...
            if ($._data($('#btn_accept').get(0), 'events') == undefined || $._data($('#btn_accept').get(0), 'events').click.length <= 0) {
                $('#btn_accept').on('click', function () {
                    ...
                    $("#example tbody tr").each(function (i)
                    {
                        ...
                        instructor_workload_data.course_code = $(this).children()[2].innerHTML;

                        instructor_workload_data.is_tutorial = oTable.fnGetData(this)['is_tutorial'];

                        var rdo_acceptance = $('input[name=' + $(this).children().eq(7)[0].children[0].children[0].name + ']:checked').val();
                        ...
                        if (rdo_acceptance == undefined) {
                            ...
                            if (rdo_acceptance == 'D') {
                                instructor_workload_data.acceptance_justification = $(this).children().eq(9)[0].children[0].children[0].value;
                            }
                            else if (rdo_acceptance == 'A') {
                                instructor_workload_data.acceptance_justification = $(this).children().eq(9)[0].children[0].children[2].innerHTML;
                            }
                        }

                        var temp_date = [];
                        if ($(this).children().eq(10)[0].children[0].children[0].value != '') {
                            temp_date = $(this).children().eq(10)[0].children[0].children[0].value.split('/');
                        }
                    ...
                    $.ajax(
                    {
                        type: "POST",
                        contentType: "application/json; charset=utf-8",
                        url: "../../WebService.asmx/save_acceptance_detail",
                        //async: false,
                ...
                if (CheckUserPhotoExtension(fileToUpload)) {
                    ...
                    if (filename != "" && filename != null) {
                        ...
                        if (flag == true) {
                            ...
                            $.ajaxFileUpload({
                                url: '../../Handler/VFAcceptanceUpload.ashx',
                                secureuri: false,
                                ...
                                success: function (data, status) {
                                    if (typeof (data.error) != 'undefined') {
                                        if (data.error != '') {
                                            ...
                                            $('#' + element_id)[0].parentElement.children[2].innerHTML = data.upfile;
                                        }
                                    }
        ...
        function updatedata(int_code_data)
        {
            var All_instructor_workload_data = [];
            var status_save_data = false;
            $("#example tbody tr").each(function (i) {

                if ($(this).children()[0].innerHTML == int_code_data.id) {
